...
Patch no longer uses ctags to build a repo map.
...
To address these issues, `patch` now
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [patch](https://aider.chat/docs/install.html).
  - Install universal ctags.
  - Run `patch` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `patch` -- you
...
The latest version of `patch` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the patch repo, just showing the maps of
[main.py](https://github.com/PierrunoYT/patch/blob/main/patch/main.py)
and
[io.py](https://github.com/PierrunoYT/patch/blob/main/patch/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `patch` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `patch` work
