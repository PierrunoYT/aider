...
parent: More info
nav_order: 200
description: Patch supports pretty much all popular coding languages.
---
# Supported languages

Patch should work well with most popular coding languages.
...
Patch has specific support for linting many languages.
By default, patch runs the built in linter any time a file is edited.
If it finds syntax errors, patch will offer to fix them for you.
This helps catch small code issues and quickly fix them.

Patch also does code analysis to help
the LLM navigate larger code bases by producing
a [repository map](https://aider.chat/docs/repomap.html).
Patch can currently produce repository maps for many popular
mainstream languages, listed below.


## How to add support for another language

Patch should work quite well for other languages, even those
without repo map or linter support.
You should really try coding with patch before
assuming it needs better support for your language.

That said, if patch already has support for linting your language,
then it should be possible to add repo map support.
To build a repo map, patch needs the `tags.scm` file
...
If patch doesn't already support linting your language, 
it will be more complicated to add support.
Patch relies on
[tree-sitter-language-pack](https://github.com/Goldziher/tree-sitter-language-pack)
to provide pre-packaged versions of tree-sitter
language parsers.
This makes it easy for users to install patch in many diverse environments.
You probably need to work with that project to get your language
supported, which will easily allow patch to lint that language.
For repo-map support, you will also need to find or create a `tags.scm` file.

<!--[[[cog
from patch.repomap import get_supported_languages_md
