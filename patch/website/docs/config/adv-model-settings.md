...
In most cases, you can safely ignore patch's warning about unknown context
window size and model costs.

{: .note }
Patch never *enforces* token limits, it only *reports* token limit errors
from the API provider.
You probably don't need to
configure patch with the proper token limits
for unusual models.

But, you can register context window limits and costs for models that aren't known
to patch. Create a `.patch.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch patch.
...
in the `.patch.model.metadata.json` file.
...
Patch has a number of settings that control how it works with
different models.
...
a model that patch doesn't know about.


### Configuration file locations

You can override or add settings for any model by creating a `.patch.model.settings.yml` file in one of these locations:
...
You can use the special model name `patch/extra_params` to define 
...
`patch/extra_params` settings taking precedence for any direct conflicts.
...
[models.py](https://github.com/PierrunoYT/patch/blob/main/patch/models.py)
file for more details about all of the model setting that patch supports.
...
<!--[[[cog
from patch.models import get_model_settings_as_yaml
cog.out("```yaml\n")
