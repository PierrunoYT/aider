...
If you specify a model that patch has never heard of, you will get
this warning.
This means patch doesn't know the context window size and token costs
for that model.
Patch will use an unlimited context window and assume the model is free,
...
If patch isn't familiar with the model you've specified,
...
Patch is unable verify the environment because it doesn't know
which variables are required for the model.
If required variables are missing,
you may get errors when you attempt to chat with the model.
You can look in the [patch's LLM documentation](/docs/llms.html)
or the
