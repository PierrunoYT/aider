...
Patch can connect to models provided by Amazon Bedrock.
To configure Patch to use the Amazon Bedrock API, you need to set up your AWS credentials.
...
Once your AWS credentials are set up, you can run Patch with the `--model` command line switch, specifying the Bedrock model you want to use:

```bash
# Change directory into your codebase
cd /to/your/project

patch --model bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0
```

Sometimes it seems to help if you prefix the model name with "us.":

```bash
patch --model bedrock/us.anthropic.claude-3-5-sonnet-20240620-v1:0
```
...
```bash
patch --list-models bedrock/
```

Make sure you have access to these models in your AWS account before attempting to use them with Patch.
...
uv tool run --from patch-code pip install boto3

# Or with pipx...
pipx inject patch-code boto3
