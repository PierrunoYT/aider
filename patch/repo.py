...
from patch import prompts, utils
...
class GitRepo:
    repo = None
    patch_ignore_file = None
    patch_ignore_spec = None
    patch_ignore_ts = 0
    patch_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if patch_ignore_file:
            self.patch_ignore_file = Path(patch_ignore_file)

    def commit(self, fnames=None, context=None, message=None, patch_edits=False, coder=None):
        """
        ...
        Args:
            ...
            patch_edits (bool, optional): Whether the changes were made by Patch. Defaults to False.
        ...
        Patch generated the changes (`patch_edits`).
        ...
        - patch_edits=True: Changes were generated by Patch (LLM).
        - patch_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (patch)".
        - --attribute-committer: Modify Committer name to "User Name (patch)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: patch (<model>) <patch@localhost>" trailer to commit message.

        Behavior Summary:

        1. When patch_edits = True (AI Changes):
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(patch), Committer=You, Trailer added
        - User commit (defaults): patch_edits=False -> Author=You, Committer=You(patch)
        ...
        prefix_commit_message = patch_edits and (
        ...
        if patch_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: patch ({model_name}) <patch@localhost>"

        # Determine if author/committer names should be modified
        # Author modification applies only to patch edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            patch_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of patch_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an patch edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (patch_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        committer_name = f"{original_user_name} (patch)"
    ...
    def refresh_patch_ignore(self):
        if not self.patch_ignore_file:
            return

        current_time = time.time()
        if current_time - self.patch_ignore_last_check < 1:
            return

        self.patch_ignore_last_check = current_time

        if not self.patch_ignore_file.is_file():
            return

        mtime = self.patch_ignore_file.stat().st_mtime
        if mtime != self.patch_ignore_ts:
            self.patch_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.patch_ignore_file.read_text().splitlines()
            self.patch_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        return self.patch_ignore_spec.match_file(fname)
