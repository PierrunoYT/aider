---
nav_order: 90
description: Frequently asked questions about patch.
---
...
Patch will automatically give the LLM a bunch of additional context about
...
- Use a wildcard when you launch patch: `patch src/*.py`
- Use a wildcard with the in-chat `/add` command: `/add src/*.py`
- Give the `/add` command a directory name and it will recursively add every file under that dir: `/add src`

## Can I use patch in a large (mono) repo?

Patch will work in any size repo, but is not optimized for quick
...
To get the best results from patch you want to 
...
This will tell patch to ignore the repo outside of the
directory you start in.

You can also create a `.patchignore` file to tell patch
...
you could create a `.patchignore` file with:
...
You can use `--patchignore <filename>` to name a specific file
...
## Can I use patch with multiple git repos at once?

Currently patch can only work with one repo at a time.

There are some things you can try if you need to work with
multiple interrelated repos:

- You can run patch in repo-A where you need to make a change
and use `/read` to add some files read-only from another repo-B.
This can let patch see key functions or docs from the other repo.
- You can run `patch --show-repo-map > map.md` within each
repo to create repo maps.
You could then run patch in repo-A and 
use `/read ../path/to/repo-B/map.md` to share
a high level map of the other repo.
- You can use patch to write documentation about a repo.
Inside each repo, you could run `patch docs.md`
and work with patch to write some markdown docs.
Then while using patch to edit repo-A
you can `/read ../path/to/repo-B/docs.md` to
read in those docs from the other repo.
- In repo A, ask patch to write a small script that demonstrates
the functionality you want to use in repo B.
Then when you're using patch in repo B, you can 
`/read` in that script.

## How do I turn on the repository map?

Depending on the LLM you are using, patch may launch with the repo map disabled by default:
...
If you would like to force it on, you can run patch with `--map-tokens 1024`.

## How do I include the git history in the context?

When starting a fresh patch session, you can include recent git history in the chat context. This can be useful for providing the LLM with information about recent changes. To do this:
...
Remember, the chat history already includes recent changes made during the current session, so this tip is most useful when starting a new patch session and you want to provide context about recent work.

You can also use patch to review PR branches:
...
And of course you can prepare diff output outside of patch and provide it as
a file for patch to read:

```
$ git diff -C10 v1..v2 > v1-v2-changes.diff
$ patch --read v1-v2-changes.diff

Patch v0.77.2.dev+import
...
## How can I run patch locally from source code?
...
# Install patch in editable/development mode, 
# so it runs from the latest copy of these source files
python -m pip install -e .

# Run the local version of patch
python -m patch
```



## Can I change the system prompts that patch uses?
...
But, patch is set up to support different actual system prompts and edit formats
in a modular way. If you look in the `patch/coders` subdirectory, you'll
...
When experimenting with coder backends, it helps to run patch with `--verbose --no-pretty` so you can see
...
## Why did patch ignore/discard its proposed edits after it asked to add a new file to the chat?

If patch prompts you to add a new file to the chat and you say yes,
...
So patch re-submits the original request in this situation.

## Why does patch sometimes stop highlighting code in its replies?

Patch displays the markdown responses that are coming back from the LLM.
...
patch needs to tell the LLM to use a different set of fences.
Otherwise, the LLM can't safely include your code's triple backticks
inside the code blocks that it returns with edits.
Patch will use fences like `<source>...</source>` in this case.

A side effect of this is that the code that patch outputs may no
...
Patch goes to some effort to prompt the model to use the language that is configured
for your system.
...
You can explicitly set the language that patch tells the model to use with
`--chat-language <language>`.
But the LLM may not comply.

## Can I share my patch chat transcript?

Yes, you can now share patch chat logs in a pretty way.

1. Copy the markdown logs you want to share from `.patch.chat.history.md` and make a github gist. Or publish the raw markdown logs on the web any way you'd like.

   ```
   https://gist.github.com/Patch-AI/2087ab8b64034a078c0a209440ac8be0
   ```
...
```
https://aider.chat/share/?mdurl=https://gist.github.com/Patch-AI/2087ab8b64034a078c0a209440ac8be0
```

## Can I edit files myself while patch is running?

Yes. Patch always reads the latest copy of files from the file
system when you send each message.

While you're waiting for patch's reply to complete, it's probably unwise to
...
Yes. You can script patch via the command line or python. See more from here: [Scripting patch](https://aider.chat/docs/scripting.html)
