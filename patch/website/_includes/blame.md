...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var blameData = {
        labels: labels,
        datasets: [{
            label: 'Patch\'s percent of new code by release',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.patch_percentage }}, lines: {{ row.patch_total }}, end_date: '{{ row.end_date }}' },{% endfor %}],
    ...
    var linesData = {
        ...
        datasets: [{
            label: 'Patch',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.patch_total }} },{% endfor %}],
            backgroundColor: 'rgba(54, 162, 235, 0.8)',
            borderColor: 'rgba(54, 162, 235, 1)',
            borderWidth: 1
        },
        {
            label: 'Human',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.total_lines | minus: row.patch_total }} },{% endfor %}],
    ...
    var blameChart = new Chart(blameCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'Patch\'s contribution';
                ...
                title: {
                    display: true,
                    text: 'Percent of new code written by patch, by release',
                    font: {
                        size: 16
                    }
                }
