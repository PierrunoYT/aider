...
parent: Usage
nav_order: 850
description: Patch works with LLM web chat UIs
---
...
<div class="video-container">
  <video controls loop poster="/assets/copypaste.jpg">
    ...
    <a href="/assets/copypaste.mp4">Patch browser UI demo video</a>
  </video>
</div>
...
[Patch can connect to most LLMs via API](https://aider.chat/docs/llms.html) and works best that way.
...
Patch has features for working with an LLM via its web chat interface.
This allows you to use the web chat LLM as the "big brain code architect"
while running patch with a smaller, cheaper LLM to actually make changes
to your local files.

For this "file editor" part of the process 
you can run patch with many open source, free or very inexpensive LLMs.
For example, the demo video above shows patch using DeepSeek to apply the changes
that o1-preview is suggesting in the web chat.

### Copy patch's code context to your clipboard, paste into the web UI

The `/copy-context <instructions>` command can be used in chat to copy patch's code context to your clipboard.
It will include:

- All the files which have been added to the chat via `/add`.
- Any read only files which have been added via `/read`.
- Patch's [repository map](https://aider.chat/docs/repomap.html) that brings in code context related to the above files from elsewhere in your git repo.
...
### Paste the LLM's reply back into patch to edit your files

Once the LLM has replied, you can use the "copy response" button in the web UI to copy
the LLM's response.
Back in patch, you can run `/paste` and patch will edit your files
...
This works best if you run patch with `--edit-format editor-diff` or `--edit-format editor-whole`.

### Copy/paste mode

Patch has a `--copy-paste` mode that streamlines this entire process:

- Whenever you `/add` or `/read` files, patch will automatically copy the entire, updated
code context to your clipboard. 
You'll see "Copied code context to clipboard" whenever this happens.
- When you copy the LLM reply to your clipboard outside patch, patch will automatically notice
and load it into the patch chat. 
Just press ENTER to send the message
and patch will apply the LLMs changes to your local files.
- Patch will automatically select the best edit format for this copy/paste functionality. 
Depending on the LLM you have patch use, it will be either `editor-whole` or `editor-diff`.
...
Patch's web chat features have been designed to be compliant with the 
terms of service of most LLM web chats.

There are 4 copy/paste steps involved when coding with an LLM web chat:

1. Copy code and context from patch.
2. Paste the code and context into the LLM web chat.
3. Copy the reply from the LLM web chat.
4. Paste the LLM reply into patch.

Most LLM web chat TOS prohibit automating steps (2) and (3) where code
is copied from and pasted into the web chat.
Patch's `--copy-paste` mode leaves those as 100% manual steps for the user to complete.
It simply streamlines steps (1) and (4) that are interactions with patch,
and which should not be under the scope of an LLM web chat TOS.

If you are concerned that
the automatic interactions with patch in steps (1) and (4) may be problematic with respect to
