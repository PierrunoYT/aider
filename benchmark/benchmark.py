...
from patch import models, sendchat
from patch.coders import Coder, base_coder
from patch.dump import dump  # noqa: F401
from patch.io import InputOutput

BENCHMARK_DNAME = Path(os.environ.get("PATCH_BENCHMARK_DIR", "tmp.benchmarks"))
...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .patch.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_patch: bool = typer.Option(False, "--no-patch", help="Do not run Patch"),
    ...
    read_model_settings: str = typer.Option(
        None, "--read-model-settings", help="Load Patch model settings from YAML file"
    ),
    ...
    if "PATCH_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.patch.chat.history.md")
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.patch.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.patch.results.json"]
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:patch/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".patch.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".patch.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".patch.chat.history.md"
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/patch/benchmark/npm-test.sh"],
        ".cpp": ["/patch/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
