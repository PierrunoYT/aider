...
In these cases, patch might say something like "Failed to apply edit to *filename*"
or other error messages.

This usually happens because the LLM is disobeying the system prompts
and trying to make edits in a format that patch doesn't expect.
Patch makes every effort to get the LLM
...
Patch also sends the LLM a [map of your entire git repo](https://aider.chat/docs/repomap.html), so other relevant code will be included automatically.
...
Most local models are just barely capable of working with patch,
...
because they are not capable enough to follow patch's system prompts.

## Try the whole edit format

Run patch with `--edit-format whole` if were using a different edit format.
...
Run patch with `--architect` or `/chat-mode architect` to enable [architect mode](../usage/modes.md#architect-mode-and-the-editor-model).
