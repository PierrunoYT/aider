...
Patch can automatically lint and test your code
...
Patch comes with built in linters for 
...
If there are linting errors, patch expects the
command to print them on stdout/stderr
and return a non-zero exit code.
This is how most linters normally operate.

By default, patch will lint any files which it edits.
...
confuse patch into thinking there's an actual lint error that needs to be fixed.
...
Patch will run the test command without any arguments.
If there are test errors, patch expects the
command to print them on stdout/stderr
and return a non-zero exit code.

Patch will try and fix any errors
if the command returns a non-zero exit code.

You can configure patch to run your test suite
...
If you want to have patch compile code after each edit, you
...
and optionally share the output with patch.
This can be useful to share error messages or to show patch
