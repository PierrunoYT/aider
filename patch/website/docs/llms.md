...
nav_order: 40
has_children: true
description: Patch can connect to most LLMs for AI pair programming.
---

# Patch can connect to most LLMs
{: .no_toc }
...
Patch works best with these models, which are skilled at editing code:
...
Patch works with a number of **free** API providers:

- [OpenRouter offers free access to many models](https://openrouter.ai/models/?q=free), with limitations on daily usage.
- Google's [Gemini 2.5 Pro Exp](/docs/llms/gemini.html) works very well with patch.

## Local models
{: .no_toc }

Patch can work also with local models, for example using [Ollama](/docs/llms/ollama.html).
It can also access
...
Models weaker than GPT 3.5 may have problems working well with patch.
