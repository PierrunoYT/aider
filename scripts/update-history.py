...
def get_latest_version_from_history():
    ...
    match = re.search(r"### (?:Patch|Aider) v(\d+\.\d+\.\d+)", history_content)
    if not match:
...
def main():
    ...
    version_match = re.search(
        rf"^### (?:Patch|Aider) v{re.escape(latest_ver)}$", history_content, re.MULTILINE
    )
    if not version_match:
        raise ValueError(f"Could not find version header for v{latest_ver}")
    version_header = version_match.group(0)
    ...
    next_version_match = re.search(
        r"^### (?:Patch|Aider) v\d+\.\d+\.\d+$",
        history_content[version_idx + len(version_header) :],
        re.MULTILINE,
    )
    next_version_idx = (
    ...
    # Run blame to get Patch percentage
    blame_result = subprocess.run(["python3", "scripts/blame.py"], capture_output=True, text=True)
    patch_line = blame_result.stdout.strip().split("\n")[-1]  # Get last line with percentage

    # Construct and run the Patch command
    message = history_prompt.format(patch_line=patch_line)
    ...
    # Run update-docs.sh after Patch
