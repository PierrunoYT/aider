...
class TestMain(TestCase):
    def setUp(self):
        ...
        # Fake home directory prevents tests from using the real ~/.patch.conf.yml file:
    ...
    @patch("patch.repo.GitRepo.get_commit_message", return_value="mock commit message")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".patch.conf.yml").write_text("auto-commits: false\n")
        with patch("patch.coders.Coder.create") as MockCoder:
            main(["--yes"], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".patch.conf.yml").write_text("auto-commits: true\n")
        with patch("patch.coders.Coder.create") as MockCoder:
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because patch will try and `git add` a file that's already in the repo.
    ...
    def test_setup_git(self):
        ...
        self.assertEqual(".patch*", gitignore.read_text().splitlines()[0])
    ...
    def test_encodings_arg(self):
        ...
        with GitTemporaryDirectory():
            with patch("patch.coders.Coder.create") as MockCoder:  # noqa: F841
                with patch("patch.main.InputOutput") as MockSend:
    ...
    def test_main_exit_calls_version_check(self):
        with GitTemporaryDirectory():
            with (
                patch("patch.main.check_version") as mock_check_version,
                patch("patch.main.InputOutput") as mock_input_output,
            ):
                main(["--exit", "--check-update"], input=DummyInput(), output=DummyOutput())
                mock_check_version.assert_called_once()
                mock_input_output.assert_called_once()

    @patch("patch.main.InputOutput")
    @patch("patch.coders.base_coder.Coder.run")
    ...
    def test_dark_mode_sets_code_theme(self):
        ...
        with patch("patch.main.InputOutput") as MockInputOutput:
    ...
    def test_env_file_flag_sets_automatic_variable(self):
        env_file_path = self.create_env_file(".env.test", "PATCH_DARK_MODE=True")
        with patch("patch.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
    ...
    def test_default_env_file_sets_automatic_variable(self):
        self.create_env_file(".env", "PATCH_DARK_MODE=True")
        with patch("patch.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
            MockInputOutput.return_value.get_input.confirm_ask = True
    ...
    def test_false_vals_in_env_file(self):
        self.create_env_file(".env", "PATCH_SHOW_DIFFS=off")
        with patch("patch.coders.Coder.create") as MockCoder:
    ...
    def test_true_vals_in_env_file(self):
        self.create_env_file(".env", "PATCH_SHOW_DIFFS=on")
        with patch("patch.coders.Coder.create") as MockCoder:
            main(["--no-git", "--yes"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_lint_option(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            with patch("patch.linter.Linter.lint") as MockLinter:
    ...
    def test_verbose_mode_lists_env_vars(self):
        self.create_env_file(".env", "PATCH_DARK_MODE=on")
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            ...
            relevant_output = "\n".join(
                ...
                if "PATCH_DARK_MODE" in line or "dark_mode" in line
            )  # this bit just helps failing assertions to be easier to read
            self.assertIn("PATCH_DARK_MODE", relevant_output)
            self.assertIn("dark_mode", relevant_output)
            self.assertRegex(relevant_output, r"PATCH_DARK_MODE:\s+on")
    ...
    def test_yaml_config_file_loading(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create .patch.conf.yml files in different locations
            home_config = fake_home / ".patch.conf.yml"
            git_config = git_dir / ".patch.conf.yml"
            cwd_config = cwd / ".patch.conf.yml"
            named_config = git_dir / "named.patch.conf.yml"
            ...
            with (
                ...
                patch("patch.coders.Coder.create") as MockCoder,
            ):
    ...
    def test_map_tokens_option(self):
        with GitTemporaryDirectory():
            with patch("patch.coders.base_coder.RepoMap") as MockRepoMap:
    ...
    def test_model_metadata_file(self):
        ...
        with GitTemporaryDirectory():
            metadata_file = Path(".patch.model.metadata.json")
    ...
    def test_accepts_settings_warnings(self):
        ...
        with GitTemporaryDirectory():
            ...
            with (
                patch("patch.io.InputOutput.tool_warning") as mock_warning,
                patch("patch.models.Model.set_thinking_tokens") as mock_set_thinking,
            ):
                main(
                    [
                        "--model",
            ...
            with (
                patch("patch.io.InputOutput.tool_warning") as mock_warning,
                patch("patch.models.Model.set_reasoning_effort") as mock_set_reasoning,
            ):
                main(
    ...
    def test_no_verify_ssl_sets_model_info_manager(self, mock_set_verify_ssl):
        with GitTemporaryDirectory():
            ...
            with patch("patch.models.Model") as mock_model:
                ...
                with patch("patch.models.fuzzy_match_models", return_value=[]):
                    main(
    ...
    def test_pytest_env_vars(self):
        ...
        self.assertEqual(os.environ.get("PATCH_ANALYTICS"), "false")
    ...
    def test_git_config_include(self):
        # Test that patch respects git config includes for user.name and user.email
        with GitTemporaryDirectory() as git_dir:
            git_dir = Path(git_dir)
            ...
            # Run patch and verify it doesn't change the git config
            main(["--yes", "--exit"], input=DummyInput(), output=DummyOutput())
    ...
    def test_git_config_include_directive(self):
        # Test that patch respects the include directive in git config
        with GitTemporaryDirectory() as git_dir:
            git_dir = Path(git_dir)
            ...
            config_after_patch = git_config.read_text()
            self.assertEqual(modified_config_content, config_after_patch)
    ...
    def test_resolve_patchignore_path(self):
        # Import the function directly to test it
        from patch.args import resolve_patchignore_path

        # Test with absolute path
        abs_path = os.path.abspath("/tmp/test/.patchignore")
        self.assertEqual(resolve_patchignore_path(abs_path), abs_path)

        # Test with relative path and git root
        git_root = "/path/to/git/root"
        rel_path = ".patchignore"
        self.assertEqual(
            resolve_patchignore_path(rel_path, git_root), str(Path(git_root) / rel_path)
        )

        # Test with relative path and no git root
        rel_path = ".patchignore"
        self.assertEqual(resolve_patchignore_path(rel_path), rel_path)
    ...
    def test_default_model_selection(self):
        with GitTemporaryDirectory():
            ...
            with patch("patch.onboarding.offer_openrouter_oauth") as mock_offer_oauth:
    ...
    def test_check_model_accepts_settings_flag(self):
        ...
        with GitTemporaryDirectory():
            ...
            with patch("patch.models.Model.set_thinking_tokens") as mock_set_thinking:
                main(
                    [
                        "--model",
                        "gpt-4o",
    ...
    def test_list_models_with_direct_resource_patch(self):
        ...
        with GitTemporaryDirectory():
            ...
            with patch("patch.main.importlib_resources.files", return_value=mock_files):
                # Capture stdout to check the output
                with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                    main(
            ...
            with patch("patch.models.Model.set_reasoning_effort") as mock_set_reasoning:
                main(
                    [
                        "--model",
                        "gpt-3.5-turbo",
    ...
    def test_model_accepts_settings_attribute(self):
        with GitTemporaryDirectory():
            ...
            with patch("patch.models.Model") as MockModel:
    ...
    def test_load_dotenv_files_override(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create fake home and .patch directory
            fake_home = git_dir / "fake_home"
            fake_home.mkdir()
            patch_dir = fake_home / ".patch"
            patch_dir.mkdir()

            # Create oauth keys file
            oauth_keys_file = patch_dir / "oauth-keys.env"
