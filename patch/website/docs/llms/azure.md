...
Patch can connect to the OpenAI models on Azure.
...
Start working with patch and Azure on your codebase:

```bash
# Change directory into your codebase
cd /to/your/project

patch --model azure/<your_model_deployment_name>

# List models available from Azure
patch --list-models azure/
```

Note that patch will also use environment variables
like `AZURE_OPENAI_API_xxx`.

The `patch --list-models azure/` command will list all models that patch supports through Azure, not the models that are available for the provided endpoint.
