...
Patch works best if you have git installed.
Here are
...
and they will be loaded automatically whenever you run patch.

## Enable Playwright 

Patch supports adding web pages to the chat with the `/web <url>` command.
When you add a url to the chat, patch fetches the page and scrapes its
content.

By default, patch uses the `httpx` library to scrape web pages, but this only
...
Patch works best with all web pages if you install
...
Patch uses the [PortAudio](http://www.portaudio.com) library to
capture audio.
...
## Add patch to your IDE/editor

You can use 
[patch's `--watch-files` mode](https://aider.chat/docs/usage/watch.html)
to integrate with any IDE or editor.

There are a number of 3rd party patch plugins for various IDE/editors.
It's not clear how well they are tracking the latest
versions of patch,
so it may be best to just run the latest
patch in a terminal alongside your editor and use `--watch-files`.

### NeoVim

[joshuavial](https://github.com/joshuavial) provided a NeoVim plugin for patch:

[https://github.com/joshuavial/patch.nvim](https://github.com/joshuavial/patch.nvim)

### VS Code

You can run patch inside a VS Code terminal window.
There are a number of 3rd party 
[patch plugins for VSCode](https://marketplace.visualstudio.com/search?term=patch%20-kodu&target=VSCode&category=All%20categories&sortBy=Relevance).

### Other editors

If you are interested in creating an patch plugin for your favorite editor,
