---
title: Patch in your IDE
#highlight_image: /assets/browser.jpg
parent: Usage
nav_order: 750
description: Patch can watch your files and respond to AI comments you add in your favorite IDE or text editor.
---
...
<div class="video-container">
  <video controls loop poster="/assets/watch.jpg">
    ...
    <a href="/assets/watch.mp4">Patch browser UI demo video</a>
  </video>
</div>
...
If you run patch with `--watch-files`, it will watch all files in your repo
and look for any AI coding instructions you add using your favorite IDE or text editor.

Specifically, patch looks for one-liner comments (# ... or // ...) that either start or end with `AI`, `AI!` or `AI?` like these:
...
Patch will take note of all the comments that start or end with `AI`.
Comments that include `AI!` with an exclamation point or `AI?` with a question
mark are special.
They trigger patch to take action to collect *all* the AI comments and use them
as your instructions.

- `AI!` triggers patch to make changes to your code.
- `AI?` triggers patch to answer your question.

See the demo video above that shows patch working with AI comments in VSCode.
...
Then patch would update the file and implement the function:
...
Patch only watches for these types of **one-liner** comments:
...
Patch will look for those comment types in all files.
...
before triggering patch with a final `AI!`.
...
Just use `AI!` last, to trigger patch.
...
patch's attention to the block.
...
### Add a file to the patch chat

Rather than using `/add` to add a file inside the patch chat, you can
simply put an `#AI` comment in it and save the file.
You can undo/remove the comment immediately if you like, the file
will still be added to the patch chat.

## Also use patch chat in the terminal
...
but it can sometimes be effective to switch over to the patch terminal chat.
The chat has the history of the AI comments you just made,
so you can continue on naturally from there.

You can also use the normal patch chat in your terminal to work with
many of patch's more advanced features:
...
you can let patch infer more about the request.
...
As you use patch with your chosen LLM, you can develop a sense for how
explicit you need to make your AI comments.

## Behind the scenes

Patch sends your AI comments to the LLM with the
