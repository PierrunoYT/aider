...
Patch is available as 2 docker images:

- `paulgauthier/patch` installs the patch core, a smaller image that's good to get started quickly.
- `paulgauthier/patch-full` installs patch will all the optional extras.
...
launch the patch core container.

### Patch core 

```
docker pull paulgauthier/patch
docker run -it --user $(id -u):$(id -g) --volume $(pwd):/app paulgauthier/patch --openai-api-key $OPENAI_API_KEY [...other patch args...]
```

### Full version

```
docker pull paulgauthier/patch-full
docker run -it --user $(id -u):$(id -g) --volume $(pwd):/app paulgauthier/patch-full --openai-api-key $OPENAI_API_KEY [...other patch args...]
```
...
docker container.
Given that, you need to be in the root of your git repo for patch to be able to
