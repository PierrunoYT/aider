...
{: .note :}
This old
[patch code editing leaderboard](edit.html)
has been replaced by the
new, much more challenging
[polyglot leaderboard](/docs/leaderboards/).

[Patch's code editing benchmark](/docs/benchmarks.html#the-benchmark) asks the LLM to edit python source files to complete 133 small coding exercises
from Exercism. 
...
- **Percent using correct edit format** - Measures the percent of coding tasks where the LLM complied with the edit format specified in the system prompt. If the LLM makes edit mistakes, patch will give it feedback and ask for a fixed copy of the edit. The best models can reliably conform to the edit format, without making errors.


## Notes on the edit format

Patch uses different "edit formats" to collect code edits from different LLMs.
...
Patch is configured to use the best edit format for the popular OpenAI and Anthropic models
and the [other models recommended on the LLM page](/docs/llms.html).
For lesser known models patch will default to using the "whole" editing format
...
for information on running patch's code editing benchmarks.
...
files = [
    ...
    'patch/website/_data/edit_leaderboard.yml',
]
