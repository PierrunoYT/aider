...
Patch is configured to work with most popular reasoning models out of the box. 
You can use them like this:

```bash
# Sonnet uses a thinking token budget
patch --model sonnet --thinking-tokens 8k

# o3-mini uses low/medium/high reasoning effort
patch --model o3-mini --reasoning-effort high

# R1 doesn't have configurable thinking/reasoning
patch --model r1
```

Inside the patch chat, you can use `/thinking-tokens 4k` or `/reasoning-effort low` to change
...
if you're configuring patch to work with a lesser known reasoning model or one served
via an unusual provider.

## Reasoning settings

Different models support different reasoning settings. Patch provides several ways to control reasoning behavior:
...
Not all models support these two settings. Patch uses the 
...
If you try to use a setting that a model doesn't explicitly support, Patch will warn you:
...
This configuration:
1. Tells Patch that the model accepts the `reasoning_effort` setting
2. Indicates the model does NOT accept `thinking_tokens` (since it's not listed)
3. Causes Patch to ignore any `--thinking-tokens` value passed for this model
...
1. Force Patch to apply all settings passed via command line
...
`<think>...</think>` tags, so patch's settings
...
Patch will display the thinking/reasoning output, 
but it won't be used for file editing instructions, added to the chat history, etc.
Patch will rely on the non-thinking output for instructions on how to make code changes, etc.
...
Patch is configured to work properly with popular models
...
Include settings for your new provider in `.patch.model.settings.yml` file
...
[existing model setting configuration entries](https://github.com/PierrunoYT/patch/blob/main/patch/resources/model-settings.yml)
...
Note that patch already has these settings pre-configured, but they
