...
You can define aliases when launching patch using the `--alias` option:

```bash
patch --alias "fast:gpt-4o-mini" --alias "smart:o3-mini"
```
...
Of course,
you can also define aliases in your [`.patch.conf.yml` file](/docs/config/patch_conf.html):
...
```bash
patch --model fast  # Uses gpt-4o-mini
patch --model smart  # Uses o3-mini
```
...
Patch includes some built-in aliases for convenience:
