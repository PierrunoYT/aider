...
Patch will report an error **if a model responds** indicating that
...
The token counts that patch reports are *estimates*.
...
It's best to only add the files that patch will need to *edit*
...
as 4k tokens.
If you ask patch to make a large change that affects a lot
...
Patch has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://aider.chat/docs/llms.html).
For serving local models, 
[Ollama](https://aider.chat/docs/llms/ollama.html) is known to work well with patch.

Try using patch without an API proxy server
