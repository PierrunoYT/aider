...
parent: Usage
nav_order: 760
description: Patch can notify you when it's waiting for your input.
---

# Notifications

Patch can notify you when it's done working and is
...
When enabled, patch will notify you when the LLM has finished generating a response and is waiting for your input.

## OS-Specific Notifications

Patch automatically detects your operating system and uses an appropriate notification method:
...
```bash
patch --notifications-command "your-custom-command"
```

For example, on macOS you might use:

```bash
patch --notifications-command "say 'Patch is ready'"
```
...
```bash
patch --notifications-command "apprise -b 'Patch is ready' 'slack://your-slack-webhook-token'"
```

or Discord
```bash
patch --notifications-command "apprise -b 'Patch is ready' 'discord://your-discord-webhook-token'"
```

or even to your phone via Pushbullet
```bash
patch --notifications-command "apprise -b 'Patch is ready' 'pbul://your-pushbullet-access-token'"
```
...
PATCH_NOTIFICATIONS_COMMAND=your-custom-command
```
