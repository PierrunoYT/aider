...
excerpt: Use "/help <question>" to ask for help about using patch, customizing settings, troubleshooting, using LLMs, etc.
nav_order: 30
---
...
patch will respond with helpful information.
You can ask questions about using patch, 
customizing settings, troubleshooting, using LLMs, etc.

Behind the scenes, patch has indexed all of its
...
Based on the provided documentation, here's the answer to your question about the .env file in patch:

The .env file in patch can be both global and specific to a git repository. Patch looks for .env files in multiple locations, with the following priority order:
...
Yes, you can use local LLMs with Patch. Here's a summary of the options:

1. Ollama: Patch supports using local models through Ollama.

2. OpenAI-compatible API: Patch can access local models that provide an OpenAI-compatible API.

3. Other local models: Patch uses the LiteLLM package to connect to various LLM providers, which may include options for running models locally.

However, it's important to note that Patch works best with more capable models. Less capable models may have difficulty properly returning "code edits," which could result in Patch not being able to edit your files and commit changes effectively.
