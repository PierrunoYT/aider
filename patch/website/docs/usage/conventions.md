...
parent: Usage
nav_order: 800
description: Tell patch to follow your coding conventions when it works on your code.
---
...
The easiest way to do that with patch is to simply create
...
and then we can add it to the patch chat, along with the file(s)
that we want to edit.

It's best to load the conventions file with `/read CONVENTIONS.md` 
or `patch --read CONVENTIONS.md`. 
This way it is marked as read-only, and cached if prompt caching
is enabled.

## Community contributed conventions

You can check the [patch conventions repository](https://github.com/Patch-AI/conventions)
to find or contribute conventions files.

## Always load conventions

You can also configure patch to always load your conventions file
in the [`.patch.conf.yml` config file](/docs/config/patch_conf.html):
...
> $ patch --read CONVENTIONS.md useragent.py  
>  
> Patch v0.24.2-dev  
