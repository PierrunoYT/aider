...
parent: More info
nav_order: 480
description: Patch can handle "infinite output" from models that support prefill.
---
...
Patch is able to work around this limit with models that support
"prefilling" the assistant response.
When you use patch with a model that supports prefill, you will see
...
When patch is collecting code edits from a model and
it hits the output token limit,
patch simply initiates another LLM request with the partial
response prefilled.
...
Patch supports "infinite output" for models that support "prefill",
such as:
