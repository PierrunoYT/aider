...
Patch has a few different chat modes:

- `code` - Patch will make changes to your code to satisfy your requests.
- `ask` - Patch will discuss your code and answer questions about it, but never make changes.
- `architect` - Like code mode, patch will change your files. An architect model will propose changes and an editor model will translate that proposal into specific file edits.
- `help` - Patch will answer questions about patch: usage, configuration, troubleshooting, etc.

By default, patch starts in "code" mode. As you are talking, you can
...
Or you can launch patch in one of the modes with the `--chat-mode <mode>` switch.
There is also a special shortcut `--architect` to launch in `--chat-mode architect`.

The patch prompt will indicate the active mode:
...
Use ask mode to discuss what you want to do, get suggestions or options from patch
and provide feedback on the approach.
Once patch understands the mission, switch to code mode to have it start editing
your files.
...
have patch execute on the plan you've been discussing.
...
When you are in architect mode, patch sends your requests to two models:
...
2. Patch then sends another request to an "editor model",
asking it to turn the architect's proposal into specific file editing instructions.
Patch has built in defaults to select an editor model based on your main model.
...
The editor model uses one of patch's edit formats to let the LLM
edit source files.
Patch will pick a suitable edit format by default,
...
See this article on 
[patch's architect/editor mode](/2024/09/26/architect.html)
for more details.
...
#### /help How do I use patch with ollama?

Run `patch --model ollama/<ollama-model>`.
