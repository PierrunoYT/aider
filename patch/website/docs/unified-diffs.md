...
Patch now asks GPT-4 Turbo to use
...
Patch's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using patch's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **Patch's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using patch's existing "SEARCH/REPLACE block" edit format.
- **Patch's new unified diff edit format raised June GPT-4's score to 59%**. 
...
Patch's new unified diff editing format
...
numerous variations on patch's existing editing formats,
...
patch's new editing format and refactoring benchmark.
...
The design and implementation of patch's new unified diff editing format
...
Patch's [previous benchmark results](https://aider.chat/docs/benchmarks.html) made
...
Patch tells GPT not to include line numbers,
...
Patch's system prompt encourages
...
When a patch fails, patch needs to ask GPT for a corrected version of the diff.
...
Patch tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, patch uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on patch's original Exercism benchmark.

## Refactoring benchmark

Patch has long used a
...
[9 popular open source python repositories](https://github.com/Patch-AI/refactor-benchmark)
...
A [simple python AST scanning script](https://github.com/PierrunoYT/patch/blob/main/benchmark/refactor_tools.py)
...
[benchmark suite that provokes, detects and quantifies GPT coding laziness](https://github.com/Patch-AI/refactor-benchmark).
...
patch's new unified diff format seems
...
when originally building patch.
...
incorporated into patch provide
...
patch's simple, high level style of unified diffs.
