...
Patch can connect to GitHub Copilot’s LLMs because Copilot exposes a standard **OpenAI-style**
endpoint at:
...
Each returned ID can be used with patch by **prefixing it with `openai/`**:

```bash
patch --model openai/gpt-4o
# or
patch --model openai/claude-3.7-sonnet-thought
```
...
## Optional config file (`~/.patch.conf.yml`)
...
* Calls made through patch are billed through your Copilot subscription  
  (patch will still print *estimated* costs).
* The Copilot docs explicitly allow third-party “agents” that hit this API – patch is playing by
  the rules.
* Patch talks directly to the REST endpoint—no web-UI scraping or browser automation.
