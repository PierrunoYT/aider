...
from patch.args_formatter import (
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from patch.deprecated import add_deprecated_model_args

from .dump import dump  # noqa: F401


def resolve_patchignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        prog="patch",
        description="Patch is AI pair programming in your terminal",
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="PATCH_",
    )
    ...
    from patch import coders as _patch_coders

    edit_format_choices = sorted(
        {
            c.edit_format
            for c in _patch_coders.__all__
    ...
    default_input_history_file = (
        os.path.join(git_root, ".patch.input.history") if git_root else ".patch.input.history"
    )
    default_chat_history_file = (
        os.path.join(git_root, ".patch.chat.history.md") if git_root else ".patch.chat.history.md"
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .patch.llm.history)",
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .patch* to .gitignore (default: True)",
    )
    group.add_argument(
        "--add-gitignore-files",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable/disable the addition of files listed in .gitignore to Patch's editing scope.",
    )
    default_patchignore_file = (
        os.path.join(git_root, ".patchignore") if git_root else ".patchignore"
    )

    group.add_argument(
        "--patchignore",
        metavar="PATCHIGNORE",
        type=lambda path_str: resolve_patchignore_path(path_str, git_root),
        default=default_patchignore_file,
        help="Specify the patch ignore file (default: .patchignore in git root)",
    ).complete = shtab.FILE
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute patch code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute patch commits in the git committer name (default: True). If explicitly set"
            " to True, overrides --attribute-co-authored-by precedence for patch edits."
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'patch: ' if patch authored the changes (default: False)",
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'patch: ' (default: False)",
    )
    group.add_argument(
        ...
        default=True,
        help=(
            "Attribute patch edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade patch to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between patch and web UI (default: False)",
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .patch.conf.yml in git root, cwd"
            " or home directory)"
        ),
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: patch --shell-completions bash"
        ),
    )
    ...
    if command == "md":
        ...
        if len(sys.argv) > 2:
            ...
            if shell not in shtab.SUPPORTED_SHELLS:
                ...
                sys.exit(1)
            parser = get_parser([], None)
            parser.prog = "patch"  # Set the program name on the parser
